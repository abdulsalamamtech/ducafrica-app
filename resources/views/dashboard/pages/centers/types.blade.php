            ...
            <div class="my-6 bg-white rounded-lg shadow-lg dark:bg-gray-800">
                ...
                <div class="relative p-8 overflow-x-auto shadow-md sm:rounded-lg">
                    ...
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-xl md:text-2xl font-semibold text-gray-700 dark:text-gray-300">Center Types</h2>
                        ...
                        <div id="addRoleModal" tabindex="-1" aria-hidden="true"
                            ...
                            <div class="relative w-full max-w-2xl max-h-auto bg-white dark:bg-gray-700">
                                <!-- Modal content -->
                                <form action="{{ route('center-types.store') }}" method="POST"
                                    ...
                                    <div class="p-6 space-y-6">
                                        <div>
                                            <label for="CenterType-name"
                                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                                    Name
                                                </label>
                                                <input type="text" name="name" id="CenterType-name"4
                                                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                    placeholder="center & event" required="">
                                        </div>
                                        ...
                                        <div>
                                            <label for="first-name"
                                                ...
                                                <input type="text" name="description" id="CenterType-description"
                                                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                    placeholder="this center type are for activities related to..." required="">
                                        </div>
                                    </div>
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            ...
                            {{-- Database CenterTypes --}}
                            @forelse ($center_types as $CenterType)
                                <!-- CenterType table record 2 -->
                                <tr
                                    ...
                                    <th scope="row"
                                        ...
                                        {{-- <img class="w-10 h-10 rounded-full" src="/images/default-profile.png"
                                            alt="Jese image"> --}}
                                        <div class="ps-3">
                                            <div class="text-base font-semibold">{{ $CenterType->name }}</div>
                                            <div class="font-normal text-gray-500">{{ $CenterType->description }}</div>

                                        </div>
                                    </th>
                                    <td class="px-6 py-4">
                                        {{ $CenterType->centers->count()}}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            <div class="h-2.5 w-2.5 rounded-full bg-{{ $CenterType->added_by?'green':'red' }}-500 me-2"></div> {{ $CenterType->added_by?"Active":"Inactive" }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">

                                        <button id="dropdownMenuIconButton{{ $CenterType->id }}" data-dropdown-toggle="dropdownDots{{ $CenterType->id }}"
                                            ...
                                            <!-- Dropdown menu -->
                                            <div id="dropdownDots{{ $CenterType->id }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconButton{{ $CenterType->id }}">

                                                <li class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                    <!-- Modal toggle -->
                                                    <a href="#" type="button"
                                                        data-modal-target="editRoleModal{{ $CenterType->id }}"
                                                        data-modal-show="editRoleModal{{ $CenterType->id }}"
                                ...
                                <!-- Edit CenterType modal 2 -->
                                <div id="editRoleModal{{ $CenterType->id }}" tabindex="-1" aria-hidden="true"
                                    class="fixed top-0 left-0 right-0 z-50 items-center justify-center hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                    <div class="relative w-full max-w-2xl max-h-full bg-white">
                                        <!-- Modal content -->
                                        <form action="{{ route('center-types.update', $CenterType->id) }}" method="POST"
                                            class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                            ...
                                            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                                                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                                                    Edit CenterType {{ $CenterType->id }}
                                                </h3>
                                                <button type="button"
                                                    class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                    data-modal-hide="editRoleModal{{ $CenterType->id }}">
                                                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                                        fill="none" viewBox="0 0 14 14">
                                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                            ...
                                            <div class="p-6 space-y-6">
                                                <div>
                                                    <label for="CenterType-name"
                                                        ...
                                                        <input type="text" name="name" id="CenterType-name"4
                                                            class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                            placeholder="center & event" required="" value="{{ $CenterType->name }}">
                                                </div>
                                                ...
                                                <div>
                                                    <label for="first-name"
                                                        ...
                                                        <input type="text" name="description" id="CenterType-description"
                                                            class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                            placeholder="this center type are for activities related to..." required="" value="{{ $CenterType->description }}">
                                                </div>
                                            </div>
